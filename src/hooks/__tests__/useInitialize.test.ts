...
describe("useInitialize", () => {
  ...
  it("should call initAppState and backupControllerBegin on mutate", async () => {
    ...
    expect(tauriCommands.backupControllerBegin).toHaveBeenCalled();
  });
  ...
  it("should handle initAppState errors", async () => {
    ...
    expect(tauriCommands.backupControllerBegin).not.toHaveBeenCalled();
  });

  it("should handle backupControllerBegin errors", async () => {
    vi.mocked(tauriCommands.initAppState).mockResolvedValue(undefined);
    const mockError = { type: "Internal", message: "Controller failed" };
    vi.mocked(tauriCommands.backupControllerBegin).mockRejectedValue(mockError);
