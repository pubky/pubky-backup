...
describe("formatCountdown", () => {
  it("should format minutes correctly", () => {
    ...
    expect(formatCountdown(now + 120)).toBe("Next backup in 2 minutes...");
    expect(formatCountdown(now + 60)).toBe("Next backup in 1 minute...");
  });

  it("should format seconds correctly", () => {
    const now = Math.floor(Date.now() / 1000);
    expect(formatCountdown(now + 45)).toBe("Next backup in 45 seconds...");
    expect(formatCountdown(now + 1)).toBe("Next backup in 1 second...");
  });

  it("should return syncing soon for past times", () => {
    const now = Math.floor(Date.now() / 1000);
